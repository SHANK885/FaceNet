''' Perfroms face alignment on the raw images and
    stores face thumbnails in the output directory
'''
...
def main(args):
    sleep(random.random())
    # print("Output direcotry: ", args.output_dir)
    ...
    '''
    # store some git revision info in a text file in the log direcotry
    ...
    # Add a random key to the filename to allow alignment using multiple preprocess
    random_key = np.random.randint(0, high=99999)
    bounding_boxes_filename = os.path.join(output_dir,
                                           'bounding_boxes_%05d.txt' % random_key)
    # print("bounding_boxes_filename: ", bounding_boxes_filename)
        ...
        for cls in dataset:
            ...
            if not os.path.exists(output_class_dir):
                ...
                if args.random_order:
                    ...
                    # print("shuffled image paths: ", cls.image_paths)
                    ...
                    try:
                        ...
                        if nrof_faces > 0:
                            ...
                            for i, det in enumerate(det_arr):
                                ...
                                bb[2] = np.maximum(det[2]+args.margin/2, img_size[1])
                                bb[3] = np.maximum(det[3]+args.margin/2, img_size[0])
...
def parse_arguments(argv):
    ...
    parser.add_argument('--random_order',
                        help='shuffle the order the images to enable alignemnt using multiple processes',
                        action='store_true')
